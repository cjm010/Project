...
Public Class PrintDGV
    ...
    Private Shared dgSearchResults As DataGridView                     ' Holds DataGrid Object to print its contents
    ...
    Public Shared Sub Print_DataGridView(ByVal dgv1 As DataGridView)
        ...
        Try
            ...
            dgSearchResults = dgv1

            ' Getting all Coulmns Names in the DataGridView
            AvailableColumns.Clear()
            For Each c As DataGridViewColumn In dgSearchResults.Columns
    ...
    Private Shared Sub PrintDoc_BeginPrint(ByVal sender As Object, _
        ...
        Try
            ...
            TotalWidth = 0
            For Each GridCol As DataGridViewColumn In dgSearchResults.Columns
    ...
    Private Shared Sub PrintDoc_PrintPage(ByVal sender As Object, _
        ...
        Try
            ...
            Do While RowPos <= dgSearchResults.Rows.Count - 1
                Dim GridRow As DataGridViewRow = dgSearchResults.Rows(RowPos)
                If GridRow.IsNewRow OrElse (Not PrintAllRows AndAlso Not GridRow.Selected) Then
                    RowPos += 1 : Continue Do
                End If

                CellHeight = GridRow.Height + 12
                ...
                Else
                    If NewPage Then
                        ' Draw Header
                        e.Graphics.DrawString(PrintTitle, New Font(dgSearchResults.Font, FontStyle.Bold), _
                                Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top - _
                        e.Graphics.MeasureString(PrintTitle, New Font(dgSearchResults.Font, _
                                FontStyle.Bold), e.MarginBounds.Width).Height - 13)

                        Dim s As String = Now.ToLongDateString + " " + Now.ToShortTimeString

                        e.Graphics.DrawString(s, New Font(dgSearchResults.Font, FontStyle.Bold), _
                           Brushes.Black, e.MarginBounds.Left + (e.MarginBounds.Width - _
                           e.Graphics.MeasureString(s, New Font(dgSearchResults.Font, FontStyle.Bold), _
                           e.MarginBounds.Width).Width), e.MarginBounds.Top - _
                           e.Graphics.MeasureString(PrintTitle, _
                           New Font(New Font(dgSearchResults.Font, FontStyle.Bold), FontStyle.Bold), _
    ...
    Private Shared Sub DrawFooter(ByVal e As System.Drawing.Printing.PrintPageEventArgs, ByVal RowsPerPage As Int32)
        ...
        If PrintAllRows Then
            If dgSearchResults.Rows(dgSearchResults.Rows.Count - 1).IsNewRow Then
                ' When the DataGridView doesn't allow adding rows
                cnt = dgSearchResults.Rows.Count - 2
            Else
                ' When the DataGridView allows adding rows
                cnt = dgSearchResults.Rows.Count - 1
            End If
        Else
            cnt = dgSearchResults.SelectedRows.Count
        End If

        ' Writing the Page Number on the Bottom of Page
        Dim PageNum As String = PageNo.ToString + " of " + _
                    Math.Ceiling(cnt / RowsPerPage).ToString
        e.Graphics.DrawString(PageNum, dgSearchResults.Font, Brushes.Black, _
                    e.MarginBounds.Left + (e.MarginBounds.Width - _
                    e.Graphics.MeasureString(PageNum, dgSearchResults.Font, _
